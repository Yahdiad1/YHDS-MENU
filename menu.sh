...
# Menu lengkap + payload + backup/restore + log
# ============================================================
...
UDP_LOG="/var/log/udp-custom.log"
...
# ================= BACKUP/RESTORE =================
backup_users() {
  tar -czf "$BACKUP_DIR/yhds-backup-$(date +%F).tar.gz" /etc/xray /root/udp /etc/passwd /etc/shadow /etc/group
  echo "Backup tersimpan di $BACKUP_DIR"
  read -n1
}

restore_users() {
  echo "Restore backup (pilih file):"
  ls $BACKUP_DIR
  read -p "File: " file
  tar -xzf "$BACKUP_DIR/$file" -C /
  systemctl restart xray udp-custom
...
clean_expired() {
  ...
  for u in $(awk -F: '$3>=1000 {print $1}' /etc/passwd); do
    ...
    [[ "$exp" != "never" ]] && [[ $(date -d "$exp" +%s) -lt $today ]] && userdel -f $u && echo "User $u expired dihapus"
  done
  read -n1
}
...
echo -e "${YELLOW}17) Backup Users${NC}"
echo -e "${YELLOW}18) Restore Users${NC}"
echo -e "${YELLOW}19) View UDP Logs${NC}"
echo -e "${YELLOW}20) Clean Expired Users${NC}"
}
...
while true; do
  ...
  case $x in
    ...
    6) lock_unlock ;;
    7) w; read -n1 ;;
    8) echo "Bot Telegram belum diisi"; read -n1 ;;
    9) restart_service ;;
