...
apt install -y curl wget unzip nginx socat cron netcat jq certbot screen bzip2 gzip figlet lolcat
  ...
  "inbounds": [
    {
      ...
      "streamSettings": { "network": "ws", "wsSettings": { "path": "/trojan" }, "security": "$SECURITY", "tlsSettings": { "certificates": [ { "certificateFile": "$SSL_CERT", "keyFile": "$SSL_KEY" } ] } }
    }
  ],
  "outbounds": [ { "protocol": "freedom" } ]
}
...
# MENU YHDS
# -------------------------
wget -O /usr/local/bin/menu https://raw.githubusercontent.com/Yahdiad1/YHDS-MENU/main/menu.sh
